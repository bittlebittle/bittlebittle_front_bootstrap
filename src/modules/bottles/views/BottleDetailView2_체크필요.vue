<template>
  <div class="bottle-detail-view" v-if="bottle">
    ...
    <img :src="bottle.imagePath" alt="Bottle Image" />
  </div>
    <!-- 관련 보틀 리스트 -->
<div class="related-list">
  ...
  <ul>
    <li v-for="relatedBottle in relatedBottleList" :key="relatedBottle.bottleNo" @click="pageUpdate(relatedBottle.bottleNo)">
      <img :src="relatedBottle.imagePath" alt="Related Bottle Image" />
    ...
    <b-modal v-model="editReplyModalVisible" title="댓글 수정" v-if="editReplyModalVisible">
      <div class="modal-content">
        <form @submit.prevent="saveReply">
          ...
          <br>
          <input type="file" ref="reviewImage" />
...
const addReview = async function() {
  ...
  // Adding review image if present
  if (reviewImage.value && reviewImage.value.files[0]) {
    data.append('reviewImage', reviewImage.value.files[0])
  }
  ...
  try {
    ...
    const showEditReplyModal = function (replyNo) {
  if (replyNo) {
    selectedReplyNo.value = replyNo
    editReplyModalVisible.value = true
    editReplyContent.value = replyList.value.filter(reply => reply.replyNo === replyNo)[0].replyContent
  }
}
  ...
  axios
    .post(
      `/api/bottles/${bottle.value.bottleNo}/reviews/${selectedReview.value.reviewNo}/replies/set-data`,
      data
    )
    .then((res) => {
      // 리뷰 등록 후 새로고침 없이 해당 보틀의 리뷰 리스트 갱신
